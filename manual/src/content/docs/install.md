...
- No connection to an IBM i is required to use DSPF-edit itself — it works directly on your local DDS source. A connection is only needed for the optional [Resolve Referenced Field](/guides/referenced-fields/) feature, provided by the [Code for i](https://marketplace.visualstudio.com/items?itemName=HalcyonTechLtd.code-for-ibmi) extension.
...
- Resolve the real type, length, and decimals of referenced fields (`R` fields) straight from the system — see [Referenced Fields](/guides/referenced-fields/).
- Fetch the `QDECFMT` system value to preview `EDTCDE()`-edited numeric fields using your system's decimal convention — see [Configuration](/guides/configuration/).

## Next step

Continue to the [Quick Start](/quickstart/) to open your first display file.
