...
exports.addKeyCommand = addKeyCommand;
 ...
 */
function addKeyCommand(context) {
    context.subscriptions.push(vscode.commands.registerCommand("dspf-edit.add-key", async (node) => {
 ...
 */
async function handleAddKeyCommandCommand(node) {
    try {
        ...
        if (currentKeyCommands.length > 0) {
            ...
            const action = await vscode.window.showQuickPick(['Add more key commands', 'Replace all key commands', 'Remove all key commands'], {
 ...
 */
function getCurrentKeyCommandsForRecord(element) {
    // Find the record in the fieldsPerRecords data
    const recordInfo = dspf_edit_model_1.fieldsPerRecords.find(r => r.record === element.name);
 ...
 */
function getAvailableKeyNumbers(currentKeys) {
    const allKeys = [];
 ...
 */
async function collectKeyCommandsWithIndicatorsFromUser(availableKeys) {
    const selectedKeyCommands = [];
    let remainingKeys = [...availableKeys];
    while (remainingKeys.length > 0) {
        const selectedKey = await vscode.window.showQuickPick(remainingKeys.map(key => ({
 ...
 */
async function selectCommandType() {
    const commandTypes = [
        {
            label: 'CA',
 ...
 */
async function collectKeyCommandDescription(keyNumber) {
 ...
 */
function validateKeyCommandDescription(value) {
    if (!value || value.trim() === '') {
 ...
 */
async function collectIndicatorsForKeyCommand(commandType, keyNumber) {
    const indicators = [];
 ...
 */
function createKeyCommandLineWithIndicators(keyCommandWithIndicators) {
 ...
 */
function calculateKeyCommandDeletionRanges(document, keyCommandLines) {
 ...
 */
function findExistingKeyCommandLines(editor, element) {
...
;
//# sourceMappingURL=dspf-edit.add-keys.js.map