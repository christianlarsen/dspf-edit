 ...
 */
async function handleCopyRecordCommand(node) {
    try {
        ...
        vscode.window.showInformationMessage(`Successfully copied record '${element.name}' to '${newRecordName}' (${recordBoundary.totalLines} lines copied).`);
    }
    catch (error) {
 ...
 */
function validateNewRecordName(value, originalName) {
    if (!value || value.trim() === '') {
 ...
 */
function detectRecordBoundaries(editor, element) {
 ...
 */
function findRecordEndLine(editor, startLineIndex) {
    const document = editor.document;
 ...
 */
function isRecordDefinitionLine(lineText) {
 ...
 */
function extractRecordLines(editor, copyConfig) {
    const lines = [];
 ...
 */
function modifyRecordLines(recordLines, copyConfig) {
 ...
 */
function replaceRecordNameInLine(line, newName) {
 ...
 */
function replaceRecordReferences(line, originalName, newName) {
 ...
 */
async function insertCopiedRecord(editor, recordLines) {
    if (recordLines.length === 0) {
