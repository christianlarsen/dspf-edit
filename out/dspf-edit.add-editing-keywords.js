...
exports.editingKeywords = editingKeywords;
 ...
 */
function editingKeywords(context) {
    context.subscriptions.push(vscode.commands.registerCommand("dspf-edit.add-editing-keywords", async (node) => {
 ...
 */
async function handleEditingKeywordsCommand(node) {
    try {
        ...
        if (currentEditing.length > 0) {
            const currentEditDisplay = currentEditing.map(edit => `${edit.type}(${edit.value}${edit.modifier ? ' ' + edit.modifier : ''})`).join(', ');
            const action = await vscode.window.showQuickPick(['Replace field editing', 'Remove field editing'], {
        ...
        const editingSummary = selectedEditing.map(edit => `${edit.type}(${edit.value}${edit.modifier ? ' ' + edit.modifier : ''})`).join(' + ');
        vscode.window.showInformationMessage(`Applied field editing ${editingSummary} to ${node.ddsElement.name}.`);
    }
    catch (error) {
 ...
 */
function getCurrentEditingForField(element) {
 ...
 */
function isNumericField(fieldInfo) {
 ...
 */
function isNumericFieldDetailed(fieldInfo) {
    ...
    if (fieldType === 'A') {
        ...
        const hasNumericEdit = attributes.some((attr) => /^(EDTCDE|EDTWRD)\s*\(/.test(attr.toUpperCase()));
 ...
 */
function getAvailableEditCodes() {
    return [
        // Standard codes (1-4)
 ...
 */
async function collectEditCode(fieldInfo) {
                ...
                const customSymbol = await vscode.window.showInputBox({
                    ...
                    validateInput: (value) => {
                        ...
                        if (value.trim().length > 1)
                            return 'Currency symbol should be a single character';
                        return null;
                    }
                });
 ...
 */
async function collectEditWord(fieldInfo) {
    const examples = [
    ...
    vscode.window.showInformationMessage('EDTWRD allows custom formatting patterns. Use: blanks for digits, 0 for zero suppression stop, * for asterisk fill, $ for floating currency, &  for blanks in output, CR/- for negative indicators.');
    ...
    const editWordPattern = await vscode.window.showInputBox({
        ...
        validateInput: (value) => {
            if (!value.trim())
                return 'Edit word pattern is required';
            ...
            if (!pattern)
                return 'Edit word pattern cannot be empty';
 ...
 */
async function collectEditMask(baseEdit) {
    vscode.window.showInformationMessage('EDTMSK defines protection for field parts. Use: & for protected areas (displayed but not input), blank for unprotected areas (user can modify).');
    const examples = [
    ...
    const editMaskPattern = await vscode.window.showInputBox({
        ...
        validateInput: (value) => {
            if (!value.trim())
                return 'Edit mask pattern is required';
            if (!value.startsWith('\'') || !value.endsWith('\'')) {
            ...
            const pattern = value.slice(1, -1);
            if (!pattern)
                return 'Edit mask pattern cannot be empty';
 ...
 */
async function addEditingToField(editor, element, editing) {
 ...
 */
function createEditingLine(editConfig) {
 ...
 */
async function removeEditingFromField(editor, element) {
 ...
 */
function calculateEditingDeletionRanges(document, editingLines) {
 ...
 */
function findExistingEditingLines(editor, element) {
    const editingLines = [];
...
;
//# sourceMappingURL=dspf-edit.add-editing-keywords.js.map