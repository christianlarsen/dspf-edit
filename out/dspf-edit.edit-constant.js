 ...
 * @returns Position information or null if cancelled
 */
async function getRelativePosition(recordElement) {
    ...
    const selectedConstant = await vscode.window.showQuickPick(existingConstants.map(constant => ({
        label: `${constant.text}`,
 ...
 * @returns Position information or null if cancelled
 */
async function getAbsolutePositionForRecord(recordElement) {
 ...
 */
async function getExistingConstantsInRecord(recordName) {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return [];
    const constants = [];
