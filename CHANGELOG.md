...
### Fixed
- Add Validity Check: `VALUES` accepted any text typed into it with no validation, allowing invalid DDS to be written — e.g. mixing a quoted character value with a bare number (`VALUES('A' 'B' 55)`) on a character field. Each value is now validated against the field's type and length as you type: character fields require single-quoted values no longer than the field itself, numeric fields require plain unquoted numbers, and `VALUES` is no longer offered on a floating-point field (not allowed in DDS).
- Add Field / Add Constant: adding a field or constant to a subfile detail (SFL) record never checked for overlap with its SFLCTL header's own content when done from the tree's "Add Field"/"Add Constant" commands (absolute position or relative to an existing element) — only the Screen Preview's click-to-place enforced it. Both now refuse a row that's occupied by the header, the same way the preview already did.
- Screen Preview: when a subfile pair sits in a shared `WINDOW()`, the header-overlap check was off by the window's own origin row — depending on the fix in between, this either wrongly blocked a row that was actually free right below the header, or let a field be dragged back up into the header. The check and the drag boundary now agree on the same coordinate frame.
- Parser: reverted a row/column swap that had been silently applied to every field and constant inside a subfile detail (SFL) record since v0.3.2. It was meant to correct a subfile positioning bug at the time, but actually misread the DDS reference — real DDS source has every field on one subfile line sharing the same row and differing only in column, exactly as the raw source columns already say, no swap needed. Left unnoticed, it corrupted the row/column read (not written) by Change Position, Center, Copy Field, Copy Constant, Sort Elements, Fill Constant, Window Resize, and the tree's own field/constant description text for any element inside a subfile — only the Screen Preview happened to keep rendering correctly, because it had separately grown its own local workaround for the same swap. Both the swap and that workaround are now gone.
