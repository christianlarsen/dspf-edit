...
## [0.17.1] - 2026-08-13
### Added
- Preview: a "Focus" button hides the DDS source editor (maximizing the preview's editor group) so editing can happen from the preview alone; the tree view stays visible since it lives in the Side Bar. Turning it off (or closing the preview) restores the source editor.
- Tree view: records can now be reordered by dragging one onto another (moves it right before the target), onto the "Records" header, or into empty space below the list (moves it to the end) — the whole record's block of lines (definition, fields, constants, attributes) moves together in the DDS source. Only records support this for now; fields, constants and everything else stay in place. Moving an SFL (subfile) record after its SFLCTL, or an SFLCTL before its SFL, is blocked with a warning — DDS requires the SFL to stay defined first.
- Preview: selecting a single field or constant now shows its row/column position next to "1 field selected"/"1 constant selected", and its width as well for constants.
### Fixed
- Error Message: adding two or more error messages to a field in the same operation, where the last one needed a continuation line (long message text and/or a response indicator), inserted its continuation line on the same physical line as the header instead of on its own line — producing invalid DDS. The insertion logic was advancing its anchor position per *message* instead of per *line generated*, so a stale end-of-file check suppressed the line break between a message's header and continuation lines.
- The tree/preview could silently stop refreshing after an edit (e.g. deleting a field from the tree view) whenever the preview panel had focus — the change listener compared against `vscode.window.activeTextEditor`, which isn't the DDS source editor while a non-editor part of the UI (like the preview) has focus, so the refresh was skipped even though the source document itself had changed correctly.
- Preview: with the "Focus" button on, clicking a field/constant in the preview, using the tree view's move-left/move-right buttons, or clicking a tree item all stole focus back to the (hidden) source editor, undoing the maximized preview. These now update the source editor's cursor/selection in the background instead, without surfacing it.
- Deleting a field or constant conditioned by more than 3 ANDed indicators, or by more than one OR'd condition, left its indicator continuation lines (the lines above it carrying only indicators, no name) behind as dangling, invalid DDS — the deletion range only ever covered the element's own line onward, never the continuation lines preceding it.
- `parseIndicatorsFromLine` (used by Delete Element, Color, and Attribute to read which indicators condition an existing line) read the indicator columns one position too early, swallowing the condition marker into the first slot — on any line starting a new OR'd group (marker 'O'), this silently produced zero indicators instead of the real ones. This is what broke the fix above on OR'd continuation lines specifically, and also affected how Color/Attribute displayed and rewrote indicators on existing COLOR/DSPATR keywords.
- Add Field: creating a numeric field with 0 decimal positions (e.g. a plain 1-digit option field) wrote a numeric data type/keyboard-shift code (Y/S/N/F) but left the decimal-positions entry blank — which DDS treats as CHARACTER type regardless of the type column, an invalid combination that failed compilation with CPD7408 ("Entry for decimal positions or field length not valid"). Decimal positions are now always written explicitly, including "0".
